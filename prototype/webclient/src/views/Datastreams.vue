  ...
  <div class="grid" id="app">
    <div class="url colspan2 titlebar" >
      <div class="logo">
        ...
        <div class="small logo smart_city" id="logo"></div>
      ...
      <l-map id="map" :zoom="zoom" :center="center" @click="deselect">
       <l-tile-layer :url="url" :attribution="attribution"></l-tile-layer>
           ...
           <!-- <l-wms-tile-layer
              ...
              base-url="https://mapproxy.jena.de/mapproxy/wms"
              layers="Stadtplan"
              :visible="true"
              name="Stadtplan"
        ...
        <v-marker-cluster :options="{spiderfyDistanceMultiplier:3.2,animate:true,animateAddingMarkers:true}" ref="clusterRef" v-if="points" >
        <custom-marker @click.native="(ev)=>{ev.stopImmediatePropagation();markerWasClicked(point)}"
          :marker="ret(point.location.coordinates)" v-for="point in points" :key="point['dsid']" :lat-lng="res(point.location.coordinates)">
          <!--<l-icon class-name="custom-div-icon">-->

            <div class='marker-pin' :class="{'selected':point===selected}"  >
              <div class="round">
                <div class="svg_icon dark" :class="[datastreamsbyID[point['dsid']].properties['sensorthings.datastream.type']]" v-if="datastreamsbyID[point['dsid']] && datastreamsbyID[point['dsid']].properties['sensorthings.datastream.type']">
                </div>
